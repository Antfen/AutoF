...
// start changed by paul
global $revSliderAsTheme, $revSliderVersion, $revslider_screens;
...
define( 'RS_PLUGIN_URL', THEME_URI.'/framework/plugins/revslider/');
...
add_filter('revslider_dashboard_elements', 'rs_change_db_filters', 10, 2);
function rs_change_db_filters($array, $vals){ 

// if (isset($array['rs-support'])) {
	// unset($array['rs-support']);
// }

if (isset($array['rs-validation'])) {
	unset($array['rs-validation']);
}

if (isset($array['rs-newsletter'])) {
	unset($array['rs-newsletter']);
}

if (isset($array['rs-templates'])) {
	unset($array['rs-templates']);
}
...
try{
	...
	if(is_admin()){ //load admin part
		...
		function putRevSlider($data,$putIn = ""){
			...
			$includesGlobally = RevSliderFunctions::getVal($arrValues, "includes_globally","off");
