...
function ssf_wp_add_options_page() {
	...
	global $ssf_wp_dir, $ssf_wp_base, $ssf_wp_uploads_base, $text_domain, $ssf_wp_top_nav_links, $ssf_wp_vars, $ssf_wp_version;
...
/*	$custom_marker=(trim($ssf_wp_vars['custom_marker'])!="")? "custom-marker.png" : "map-pin.png"; 

	$custom_marker_active=(trim($ssf_wp_vars['custom_marker_active'])!="")? "custom-marker-active.png" : "map-pin-active.png"; */
   ...
   if(file_exists($dir.'/custom-marker-active.png'))
	...
	/*
style_list_number_circle
	style_list_number_circle_active
style_list_number_bg_active
style_list_number_font_active
...
$marker_label_color=(trim($ssf_wp_vars['marker_label_color'])!="")? ssfParseToXML($ssf_wp_vars['marker_label_color']) : "";
	...
	/*if (function_exists('icl_t')) {
		...
		$GLOBALS['output_zone_type'] = 'ssf_wp_dyn_js';
		...
		foreach ($labels_arr as $value) {
			 ...
			 echo $the_field = $value['field_name'];

			$$the_field = addslashes(icl_t(SSF_WP_DIR, $value['label'], $$the_field));
...
if($show_search_bar!='1')
{
 ...
 $ssf_hide_input_box='display:none !important;';
	...
	print "</style>";
//var ssf_conatct_email=$ssf_conatct_email;	
	$byregion = '';
	$regionlist = '';
	...
	if($region_show=="1"){
		if ($locales=$wpdb->get_results("SELECT * FROM ".SSF_WP_REGION_TABLE." ", ARRAY_A)) { 
	 	foreach ($locales as $value) {
			$regionlist .= "<li> <label class='ssflabel'><input id='storesRegion' class='js-inputify' name='storesRegion' type='radio' value='".$value['ssf_wp_address_name']."' /> ".$value['ssf_wp_region_name']." </label></li>";			
		 }
	...
	$allCategory='';
		...
		{
			$allCategory="<div class='filter__row filter__row--services' id='filter__services'><div class='filter__toggler arrow-toggler'> $by_category</div>
<div class='filter__toggler-contents togglerify-slider' id='productsServicesFilterOptions'><div class='label__contents'> <label class='label--vertical-align  ssflabel'><input id='storesProductsServices' name='storesProductsServices' type='checkbox' value='default' /> $all_category </label></div></div></div>";
		}
		else if(empty($multiCateory) && $category_show=="1"){ 
		$allCategory="<div class='filter__row filter__row--services' id='filter__services'><div class='filter__toggler arrow-toggler'> $by_category</div>
<div class='filter__toggler-contents togglerify-slider' id='productsServicesFilterOptions'><div class='label__contents' style='margin-left: 4px;'> <label class='label--vertical-align  ssflabel'><input id='storesProductsServices' name='storesProductsServices' type='radio' value='default' /> $all_category </label></div></div></div>";
		}
...
$cmnt_user_email='';
$cmnt_readonly='';
$reviewAlert="<button name='submit' type='button' id='PostYourReviewBtn'> <span class='fa fa-plus'> </span>". $ssf_wp_vars['review_submit_button']."</button>";
...
if($rattingStatus=='user'){
 $current_user = wp_get_current_user();
	if ( 0 == $current_user->ID ) {
		 ...
		 $reviewAlert='<a href="'.$rattingURL.'"><button class="ssfAskLogin" type="button">'. $ssf_wp_vars["login_button_msg"].'</button></a>';
	} 
	else{
...
if(!empty($rattingAddon)){
 $ratingLink="<a  onclick='showCommentPopup();'  data-plugins='open-modal' data-template='modal-photo-viewer'  class='infobox__row infobox__comment'>".$ssf_wp_vars['review_and_ratings']."</a>";
...
<div class='ratingTopPanel'>
	<div class='ssf_review_store'>".$ssf_wp_vars['review_and_ratings']."</div>
...
<div><button name='submit' type='button' id='ratingSubmit'><span class='fa fa-plus'></span> ".$ssf_wp_vars['review_submit_button']."</button></div>
</div>
</div>
                              ...
                              <div class='store-locator-bottom-half' id='storeLocator__bottomHalf'>
                                 ...
                                 <div class='ssf-row ssf-panel pad medium-pad-2x' id='storeLocator__storeListRow'>
                                    <div class='ssf-column hide-for-small-down'>
                                       <h2 class='title space-bottom-3x'> $stores_near_you</h2>
                                    </div>
