...
<div class="reciewhead"><strong>'.$ssf_wp_vars['ratting_label'].': '.$row["ssf_wp_ratings_score"].'</strong></div>
<div class="reciewStar">'
.$ratingStar.
'</div>
<br>
<span>
'.$ssf_wp_vars['reviewed_by'].'
...
$result=$ssf_wp_vars['no_ratting_msg'];
}
...
for($i=0; $i<5; $i++){
 ...
 $updated_rating='<div class="reciewInfoStar"><span class="ratingResultImg">'.$ratingStar.'</span><span class="totalReviewShow">('.$query[0]["count"].' '.$ssf_wp_vars['review_label'].')</span></div>';
...
$rattingView .='</a><span class="totalReviewShow">('.$query[0]["count"].' '.$ssf_wp_vars['review_label'].')</span></div>';
...
'<span class="totalReviewShow" id="totalReviewShow'.$Id.'">('.$query[0]["count"].' '.$ssf_wp_vars['review_label'].')</span></div>';
...
<p>'.$ssf_wp_vars["no_ratting_msg"].'</p></div></li></ul>';
...
<li><div class="reviewBody">
<p>'.$ssf_wp_vars["no_ratting_msg"].'</p>
</div></li></ul>';
}
echo json_encode($ssfResponse);
}
...
$pid=intval($_POST["pid"]);
 ...
 if ($check[0]["count"] <= 0) {
	...
	if ($new_loc_id > 0) {
		$ssfResponse['message'] = $ssf_wp_vars['ratting_submit_msg'];
	  } else {
	...
	if ($ssfResponse['error'] === FALSE) {
	  $query=$wpdb->get_results("SELECT count(*) as count, AVG(ssf_wp_ratings_score) as score FROM ".SSF_WP_SOCIAL_TABLE." WHERE 1 AND ssf_wp_store_id = '".$pid."'", ARRAY_A);
		if ($query[0]["count"] > 0) {
		  ...
		  $ssfResponse['updated_rating'] = '<strong>'.$ssf_wp_vars['ratting_label'].': </strong>'.$ssf_wp_vars['no_ratting_msg'];
		}
	 
	}
	}else{
	$ssfResponse['error'] =$ssf_wp_vars['allready_voted_msg'];
	}
}
else
{
$ssfResponse['error'] =$ssf_wp_vars['reCAPTCHA_warning'];
}
...
print $query[0]["count"];
//'('.$query[0]["count"].' '.$ssf_wp_vars['review_label'].')';
}
