...
$(document).ready(function(){
	...
	if(geo_settings==1){
		...
		gmap_location_lookup(default_location,'200',''); 	
	}  
		...
		geocoder.geocode( {'address':address,'region':region}, function(results, status) {
			...
			if(status == google.maps.GeocoderStatus.OK) {
				...
				$.ajax({
					...
					data:"ajax=1&action=get_nearby_stores&distance="+distance+"&lat="+lat+"&lng="+lng+"&products="+$('#edit-products').val()+"&catlist="+g_catlist,
					success:function(msg) {
						...
						if( results.success ) {
							...
							///////////////////
								...
								var ctype='';
								var catimglist='';
								var cat_img= v.cat_img.split("#");
								for(var i=0 ; i<v.catcount; i++)
								{
									if(cat_img[i] != '' && cat_img[i] !=undefined) {
									catimglist += '<img src="'+cat_img[i]+'" style="max-width:15px; max-height:15px; padding-right: 5px;" />';
									}
								}
								if(v.cat_img!='' && v.cat_img!=undefined)
								{
								 ctype = '<div align="right">'+catimglist+'</div>';
								}
...
function getCategories()
{
	...
	var catlist = []; 
	$('#edit-products :selected').each(function(i, selected){ 
	...
	if(v.catcount>0)
{
	...
	var cat_img= v.cat_img.split("#");
	var cat_name= v.cat_name.split("#");
	for(var i=0 ; i<v.catcount; i++)
	{
	if(cat_img[i] != '' && cat_img[i] !=undefined) {
	    info_window_string += "<div class='products'><img src='"+cat_img[i]+"' style='max-width:15px; max-height:15px;' /> "+cat_name[i]+"</div>";
	}

	else if(cat_name[i]!=undefined && cat_name[i] != '')
	{
	  info_window_string += "<p class='description' style='display:block;'>"+cat_name[i]+"</p>";
