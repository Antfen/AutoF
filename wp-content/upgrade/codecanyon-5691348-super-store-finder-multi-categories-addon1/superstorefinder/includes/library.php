...
function json_stores_list($sql) {
	...
	if(!empty($stores)) {
		...
		$cat_img = '';
				$cat_name = '';
				$ex_cat= trim($v['cat_id'], ',');
				$ex_cat_arr = explode(',',$ex_cat);
				$tvalue=count($ex_cat_arr);
				$j=1;
				foreach ($ex_cat_arr as $catss) {
					$cat_upload_dir = ROOT.'admin/imgs/categories/'.$catss.'/';
				...
				if($cat_files !== FALSE && isset($cat_files[0])) {
                    if($j<$tvalue)
					{
					 $cat_img .= ROOT_URL.'admin/imgs/categories/'.$catss.'/'.$cat_files[0].'#';
					}
					else
					{
						$cat_img .= ROOT_URL.'admin/imgs/categories/'.$catss.'/'.$cat_files[0];
				...
				$cats = $db->get_rows("SELECT categories.* FROM categories WHERE categories.id='".$catss."'");
